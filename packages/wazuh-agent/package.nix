...
let
  ...
  # Source tarballs that `make deps` downloads. They go to src/external.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  postUnpack = ''
    ...
    # The vendored builds call /usr/bin/env, which the sandbox does not have.
    patchShebangs "$sourceRoot/src/external"
  '';

  postPatch = ''
    # Wazuh looks for ps only at these two absolute paths.
    substituteInPlace \
  ...
  # GCC 14 rejects these in the vendored libdb.
